...
class ConstraintSpec extends Specification {
	...
	def 'constraint: x0 - x1 <= 4' () {
		...
		then:
			...
			c.coefficients.getEntry(lp.varToIndex[lp.x0]) == 1
			c.coefficients.getEntry(lp.varToIndex[lp.x1]) == -1
	}
	...
	def 'constraint: x0 - #coef2*x1 <= 4' () {
		...
		then:		
			c.coefficients.getEntry(lp.varToIndex[lp.x0]) == -1
			c.coefficients.getEntry(lp.varToIndex[lp.x1]) == -coef2
	...
	def 'apache constraint: -x0 - x1 <= 4' () {
		...
		then:
			c.coefficients.getEntry(lp.varToIndex[lp.x0]) == -1.0
			c.coefficients.getEntry(lp.varToIndex[lp.x1]) == -1.0
	}
